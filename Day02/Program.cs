...
namespace Day02
{
    class Program
    {
        ...
        {
            string input = @"1-14 b: bbbbbbbbbbbbbbbbbbb
...
2-5 f: mmcfxtk";
            ...
            Regex pattern = new Regex(@"(\d+)-(\d+) ([a-z]): ([a-z]+)");
            ...
            {
                var matches = pattern.Matches(item);
                int min = Convert.ToInt32(matches[0].Groups[1].Value);
                int max = Convert.ToInt32(matches[0].Groups[2].Value);
                string letter = matches[0].Groups[3].Value;
                string pass = matches[0].Groups[4].Value;

                int end = pass.Replace(letter, "").Length;

                // first question
                if (pass.Length - end >= min && pass.Length - end <= max)
                {
                    validCount++;
                }
                ...
                if (pass.Length >= min)
                    if (pass[min - 1] == letter[0])
                        valid++;

                if (pass.Length >= max)
                    if (pass[max - 1] == letter[0])
                        valid++;

                if (valid == 1) validCount2++;
            }

            Console.WriteLine("Part 1 Answer: {0}", validCount);
            Console.WriteLine("Part 2 Answer: {0}", validCount2);
            Console.ReadLine();
        }
    }
}
